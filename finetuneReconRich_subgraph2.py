...
from dataset.dataset_test_rich2_subgraph2 import MolTestDatasetWrapperRich
from dataset.get_config import get_config 
import argparse
from torch_geometric.utils import  scatter, softmax

from torch_geometric.data import Data
from util import calc_acc
...
layout = {
    "recon": {
        ...
        "acc": ["Multiline", [ "acc/recon_node", "acc/recon_edge"]],
    },
}
...
class FineTune(object):
    ...
    def _step(self, model_list, data, n_iter):
        ...
        masked_x = data.x.clone().to(self.device)

        for atom_idx in masked_atom_indices:
            masked_x[atom_idx] = torch.zeros(42).to(self.device)
        ...
        if args.mask_edge:
            ...
            if len(connected_edge_indices) > 0:
                ...
                for bond_idx in connected_edge_indices:
                    masked_edge_attr[bond_idx] = torch.zeros(10).to(self.device)
                    masked_edge_attr[bond_idx, 0] = num_edge_type
            ...
            masked_data = Data(x = masked_x ,edge_index = data.edge_index, edge_attr = masked_edge_attr , x_add = data.x_add ) 
            ...
            loss_recon_node = self.criterion_recon(pred_node.float(), mask_node_label[:,0].long())
            ...
            masked_data = Data(x = masked_x ,edge_index = data.edge_index, edge_attr = data.edge_attr, x_add = data.x_add) 

            node_rep_masked, output2_masked = model(masked_data)   

            pred_node = linear_pred_atoms(node_rep_masked[masked_atom_indices])

        loss_recon_node = self.criterion_recon(pred_node.float(), mask_node_label[:,0].long())

        accuracy_node = calc_acc(pred_node, mask_node_label[:,0])
        ...
        if args.mask_edge:
            ...
            return pred, total_loss, loss, (loss_recon_node, loss_recon_edge), (accuracy_node, accuracy_edge)
        else:
            total_loss = loss + args.alpha * loss_recon_node
            return pred, total_loss, loss, (loss_recon_node, 0), (accuracy_node, 0)
    ...
    def train(self):
        ...
        if self.config['model_type'] == 'gin':
            from models.ginet_finetuneRecon_rich import GINetRecon_rich
            model = GINetRecon_rich(self.config['dataset']['task'], **self.config["model"]).to(self.device)
        ...
        for epoch_counter in range(self.config['epochs']):
            for bn, data in enumerate(train_loader):
                ...
                pred, total_loss, loss_end, loss_recon,acc  = self._step(model_list, data, n_iter)
                ...
                if n_iter % self.config['log_every_n_steps'] == 0:
                    ...
                    self.writer.add_scalar('loss_recon_node/train_loss_recon_node', loss_recon[0], epoch_counter)
                    self.writer.add_scalar('loss_recon_edge/train_loss_recon_edge', loss_recon[1], epoch_counter)
                    self.writer.add_scalar('acc/recon_node', acc[0], epoch_counter)
                    self.writer.add_scalar('acc/recon_edge', acc[1], epoch_counter)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mask_hop', type= int, default=1, help = "Masking hop for the dataset.")
