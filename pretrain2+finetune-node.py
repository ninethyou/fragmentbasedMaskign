...
from model_copy import GNN, GNN_graphpred
...
def main():
    ...
    parser.add_argument('--gnn_type', type=str, default = 'gin', help="gin/gat/gcn/graphsage")
    ...
    if args.input_model_file != '':# 정규 표현식을 사용하여 ratio 부분 추출
        match = re.search(r'_(\d+(\.\d+)?)_', args.input_model_file)
    
        
        if match:
            ratio = match.group(1)
            print(f"Extracted ratio: {ratio}")
        else:
            print("Ratio not found")
            
        dir_name =  savefilename +'finetune_mask_'+ ratio + '_'+ args.dataset + '_' + str(args.num_layer) + '_' \
    ...
    for epoch in range(1, args.epochs+1):
        ...
        print("train_loss : %f val_loss: %f" %(train_loss, train_loss, ))
    ...
    print("test auc, test_loss :", test_acc, test_loss)
    ...
    columns = ['Time', 'Seed', 'result', 'ratio' ]

    results_list = []
    current_time = datetime.now().strftime('%b%d_%H-%M-%S') # 시간을 지정 

    results_list.append([current_time, args.seed, test_acc, ratio])
    ...
    fn_base = f'{dir_name}/{args.dataset}_pretrain+downstream_node'
