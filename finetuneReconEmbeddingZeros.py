...
class FineTune(object):
    ...
    def _step(self, model_list, data, n_iter):
        ...
        masked_x = data.x.clone()
               ...
               # for atom_idx in masked_atom_indices:
        #     print(masked_x[atom_idx].shape, torch.tensor([num_atom_type, 0]).shape)
        #     print()

        masked_value = torch.tensor([num_atom_type, 0], device=self.device)
    
        for atom_idx in masked_atom_indices:
            masked_x[atom_idx] = masked_value
    ...
    def train(self):
        ...
        if self.config['model_type'] == 'gin':
            from models.ginet_finetuneRecon import GINetReconEmbeddingBias
            model = GINetReconEmbeddingBias(self.config['dataset']['task'], **self.config["model"]).to(self.device)
