...
from model import GNN, GNN_graphpred_embed
...
def train(args, model_list, device, loader, optimizer_list, epoch, alpha_l=1.0, loss_fn="sce" ):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration", )):  # tqdm 추가
        ...
        node_rep, pred = model(batch.masked_x, batch.edge_index, batch.edge_attr, batch.batch, batch.masked_atom_indices)
...
def main():
    ...
    #set up model
    model = GNN_graphpred_embed(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type)
    ...
    output_file_temp = "./pretrain2/" + "/node_emb_feature/"+ f"{args.dataset}_{args.gnn_type}_{args.mask_rate}_{args.seed}"        
    ...
    fn_base = f'{directory_name}/pretrain_node_emb_feature_{args.dataset}_{args.split}'
