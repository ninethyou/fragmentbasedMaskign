...
def train(args, model_list, device, loader, optimizer_list, epoch, alpha_l=1.0, loss_fn="sce" ):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration", )):  # tqdm 추가
        ...
        node_rep, pred = model(batch.x, batch.edge_index, batch.edge_attr, batch.batch, batch.masked_atom_indices)
...
def main():
    ...
    output_file_temp = "./pretrain2/" + "/node_emb/"+ f"{args.dataset}_{args.gnn_type}_{args.mask_rate}_{args.seed}"        
    ...
    fn_base = f'{directory_name}/pretrain_node_emb_{args.dataset}_{args.split}'
