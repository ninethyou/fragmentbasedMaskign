...
def plot_class_frequencies_filename(pred, label, fileName):
    ...
    ax.text(0.55, 0.95, accuracy_text, transform=ax.transAxes, fontsize=12, 
            verticalalignment='top', horizontalalignment='center', )

    # Save the plot to a file
    plt.tight_layout()
    dir_name = f'./plot/{args.task_name}/{savefilename}{args.seed}/{args.mask_rate}/'
    try : 
    ...
    def _validate(self, model_list, valid_loader):
        ...
        with torch.no_grad():
            ...
            for bn, data in enumerate(valid_loader):
                ...
                if args.mask_edge:
                    total_loss, loss, loss_node, loss_edge, pred_node,mask_node_label0, pred_edge,mask_edge_label  = self._step_test([model, linear_pred_atoms, linear_pred_bonds], data, bn, data_split='valid')
                    ...
                    total_loss, loss, loss_node, loss_edge, pred_node, mask_node_label0, _, _ = self._step_test([model, linear_pred_atoms, linear_pred_bonds], data, bn, data_split='valid')
