...
class FineTune(object):
    ...
    def _step(self, model_list, data, n_iter):
        ...
        # get the prediction
        node_rep, pred, masked_atom_indices = model(data, args.mask_rate,rank_method = 'bottomk' )  # [N,C]
    ...
    def train(self):
        ...
        if self.config['model_type'] == 'gin':
            from models.ginet_finetuneRecon_rich_top import GINetRecon_rich_top
            model = GINetRecon_rich_top(self.config['dataset']['task'], **self.config["model"]).to(self.device)
