...
def train(args, model_list, device, loader, optimizer_list, epoch, alpha_l=1.0, loss_fn="sce" ):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration", )):  # tqdm 추가
        ...
        node_rep, pred = model(batch.masked_x, batch.edge_index, batch.edge_attr, batch.x_add, batch.batch)
        ...
        if loss_fn == "sce":
            ...
            loss_recon = criterion(pred_node.double()[masked_node_indices], batch.mask_node_label[:,0].long())
...
def main():
    ...
    if args.dataset == 'zinc_standard_agent':
        from dataset_pretrain_zinc_rich import MoleculeDataset
        from util import MaskAtomTestRich
        from batch import BatchMasking       
        from dataloader import DataLoaderMaskingPredTestRich
        
        
        dataset = MoleculeDataset(data_path = "dataset - rich/" + args.dataset, )
        
        # transform = MaskAtomTestRich(num_atom_type = 119, num_edge_type = 5, mask_rate = args.mask_rate, mask_edge=args.mask_edge) 
        ...
        train_loader = DataLoaderMaskingPredTestRich(dataset, batch_size=args.batch_size, shuffle=True, drop_last = True, num_workers = args.num_workers,  mask_rate=args.mask_rate, mask_edge=args.mask_edge, pin_memory = True, prefetch_factor  = 2)
    ...
    output_file_temp = "./pretrain2/" + "/rich/" + f"{args.dataset}_{args.gnn_type}_{args.mask_rate}_{args.seed}"        
    ...
    fn_base = f'{directory_name}/pretrain_rich_{args.dataset}_{args.split}'
