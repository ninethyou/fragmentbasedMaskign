...
from dataset.dataset_test_fragment import MolTestDatasetWrapper
...
class FineTune(object):
    ...
    def _step(self, model_list, data, n_iter):
        ...
        for atom_idx in all_node_indices:
        ...
        if args.mask_edge:
            ...
            if len(connected_edge_indices) > 0:
                ...
                mask_edge_label = torch.cat(mask_edge_labels_list, dim=0).to(self.device)
                # modify the original bond features of the bonds connected to the mask atoms

                connected_edge_indices = torch.tensor(
                    connected_edge_indices[::2])
            else:
                mask_edge_label = torch.empty((0, 2)).to(torch.int64).to(self.device)
                connected_edge_indices = torch.tensor(
            ...
            for edge_idx in connected_edge_indices:
                masked_edge_attr[edge_idx] = torch.tensor([num_edge_type, 0]).to(self.device)
            ...
            node_repre2, output2_masked= model(masked_data, all_node_indices)
            
            pred_node = linear_pred_atoms(node_repre2[all_node_indices])
            loss_recon_node = self.criterion_recon(pred_node.float(), mask_node_label[:,0])
