...
layout = {
    "recon": {
        ...
        "loss_recon_node" : ["Multiline", ["loss_recon_node/train", "loss_recon_node/validation"]],
        "loss_recon_edge" : ["Multiline", ["loss_recon_edge/train", "loss_recon_edge/validation"]],
...
def calc_acc_plot_filename(pred, label, filename):
            # edge prediction   
        _, predicted_classes = torch.max(pred, dim=1) # dim=1은 노드별로 최댓값을 찾음
...
def plot_class_frequencies_filename(predicted_classes, label, filename):
    ...
    plt.tight_layout()
    dir_name = f'./{args.task_name}/{args.seed}'
...
class FineTune(object):
    ...
    def _step_test(self, model_list, data, n_iter,data_split = 'validate'):
        # get the prediction
        ...
        for atom_idx in data.masked_atom_indices:
            data_recon.x[atom_idx] = torch.tensor([num_atom_type, 0]).to(self.device)


        if args.mask_edge:

            for bond_idx in data.connected_edge_indices:
                    data_recon.edge_attr[bond_idx] = torch.tensor(
                        [self.num_edge_type, 0])
            
            node_rep_masked, output2_masked = model(data_recon)
        ...
        calc_acc_plot_filename(pred_node, data.mask_node_label[:,0], filename = f'{data_split}/{n_iter}.png')
        ...
        if args.mask_edge:
            ...
            return total_loss, loss, (loss_recon_node, loss_recon_edge), pred_node, data.mask_node_label[:,0], pred_edge, mask_edge_label[:,0]

        
        else: 
            total_loss = loss + args.alpha * loss_recon_node
            return total_loss, loss, (loss_recon_node, 0), pred_node, data.mask_node_label[:,0], 0, 0
    ...
    def _step(self, model_list, data, n_iter, data_split = 'train'):
        ...
        for atom_idx in data.masked_atom_indices:
            masked_x.x[atom_idx] = torch.tensor([num_atom_type, 0]).to(self.device)
        ...
        if args.mask_edge:
            ...
            for bond_idx in data.connected_edge_indices:
                    masked_x.edge_attr[bond_idx] = torch.tensor(
                        [self.num_edge_type, 0])
            ...
            accuracy_edge = calc_acc(pred_edge, mask_edge_label[:,0])
        ...
        # accuracy_node = calc_acc(pred_node, data.mask_node_label[:,0])
        accuracy_node = calc_acc_plot_filename(pred_node, data.mask_node_label[:,0], filename = f'{data_split}/{n_iter}.png')
        ...
        if args.mask_edge:
            ...
            return  total_loss, loss, loss_recon_node, loss_recon_edge, (accuracy_node, accuracy_edge)
            ...
            return  total_loss, loss, loss_recon_node, 0, (accuracy_node, 0)
    ...
    def train(self):
        ...
        # model_recon = copy.deepcopy(model).to(self.device)
        ...
        # optimzer_model_recon  = torch.optim.Adam(model_recon.parameters(), lr=self.config['init_lr'], weight_decay=args.weight_decay)
        ...
        # model_list = [model, model_recon, linear_pred_atoms, linear_pred_bonds]
        # optimizer_list = [optimizer_model, optimzer_model_recon, optimizer_linear_pred_atoms, optimizer_linear_pred_bonds]
        ...
        for epoch_counter in range(self.config['epochs']):
            for bn, data in enumerate(train_loader):
                ...
                total_loss, loss_end, loss_recon_node, loss_recon_edge, acc= self._step(model_list, data, n_iter, data_split = 'train')
                ...
                if n_iter % self.config['log_every_n_steps'] == 0:
                    ...
                    self.writer.add_scalar('loss_recon_node/train_loss_recon_node', loss_recon_node, epoch_counter)
                    self.writer.add_scalar('loss_recon_edge/train_loss_recon_edge', loss_recon_edge, epoch_counter)
                    self.writer.add_scalar('acc/acc_node', acc[0], epoch_counter)
                    self.writer.add_scalar('acc/acc_edge', acc[1], epoch_counter)
                    self.writer.add_scalar('alpha', args.alpha, epoch_counter)
                ...
                n_iter += 1
            print(f'{epoch_counter}, {bn}, {total_loss.item():.4f}. loss_end: {loss_end.item():.4f}, loss_recon_node: {loss_recon_node:.4f}, loss_recon_edge: {loss_recon_edge:.4f},acc_node: {acc[0]:.4f}, acc_edge: {acc[1]:.4f}')
            ...
            if epoch_counter % self.config['eval_every_n_epochs'] == 0:
                if self.config['dataset']['task'] == 'classification': 
                    valid_loss, valid_cls, valid_cls_node, valid_cls_edge, loss_recon  = self._validate(model_list, valid_loader)
                    ...
                    valid_loss, valid_rgr ,valid_cls_node, valid_cls_edge, loss_recon = self._validate(model_list, valid_loader)
                ...
                self.writer.add_scalar('loss_recon_node/validation', loss_recon[0], epoch_counter) 
                self.writer.add_scalar('loss_recon_edge/validation', loss_recon[1], epoch_counter)
    ...
    def _validate(self, model_list, valid_loader):
        ...
        with torch.no_grad():
            ...
            for bn, data in enumerate(valid_loader):
                ...
                if args.mask_edge:
                    total_loss, loss,(loss_recon_node, loss_recon_edge),  pred_node, mask_node_label0, pred_edge,mask_edge_label  \
                        = self._step_test(model_list, data, bn, data_split = 'validate')
                ...
                valid_loss += total_loss.item() * data.y.size(0)
        ...
        if self.config['dataset']['task'] == 'regression':
            ...
            if self.config['task_name'] in ['qm7', 'qm8', 'qm9']:
                ...
                print('Validation loss:', valid_loss, 'MAE:', mae,  'edge acc:', accuracy_edge, 'node acc:', accuracy_node)

                return valid_loss, mae, accuracy_node, accuracy_edge, (loss_recon_node, loss_recon_edge)
            else:
                rmse = root_mean_squared_error(labels, predictions, )
                print('Validation loss:', valid_loss, 'RMSE:', rmse, 'edge acc:', accuracy_edge, 'node acc:', accuracy_node)
                return valid_loss, rmse, accuracy_node, accuracy_edge, (loss_recon_node, loss_recon_edge)
            ...
            print('Validation loss:', valid_loss, 'ROC AUC:', roc_auc, 'edge acc:', accuracy_edge, 'node acc:', accuracy_node)

            return valid_loss, roc_auc, accuracy_node, accuracy_edge, (loss_recon_node, loss_recon_edge)
    ...
    def _test(self, model_list, test_loader):
        ...
        with torch.no_grad():
            ...
            for bn, data in enumerate(test_loader):
                ...
                if args.mask_edge:
                    total_loss, loss, (loss_recon_node, loss_recon_edge),  pred_node, mask_node_label0, pred_edge,mask_edge_label  \
                        = self._step_test(model_list, data, bn, data_split = 'test')
                    labels_node.append(mask_node_label0)
                    predictions_node.append(pred_node)
...
def main(config):
    ...
    if config['dataset']['task'] == 'classification':
        return fine_tune.roc_auc, fine_tune.acc_node, fine_tune.acc_edge
    if config['dataset']['task'] == 'regression':
        if config['task_name'] in ['qm7', 'qm8', 'qm9']:
            return fine_tune.mae,fine_tune.acc_node, fine_tune.acc_edge
        else:
            return fine_tune.rmse,fine_tune.acc_node, fine_tune.acc_edge
...
if __name__ == "__main__":
    ...
    result, acc_node, acc_edge = main(config)

    columns = ['Time', 'Seed', 'result','result_node','result_edge', 'mask_rate']
    

    results_list.append([current_time, seed, result,  acc_node, acc_edge, args.mask_rate])
