...
class FineTune(object):
    ...
    def _step( model_list, data, n_iter):
        ...
        # get the prediction
        node_rep, pred,node_top = model(data)  # [N,C]
        ...
        if args.mask_edge:
            ...
            loss_recon_node = criterion_recon(pred_node.float(), mask_node_label[:,0])
    ...
    def train():
        ...
        config['model']['num_task'] = len(config['dataset']['target'])
        if config['model_type'] == 'gin':
            from models.ginet_finetuneReconTop import GINetReconEmbedding
            model = GINetReconEmbedding(config['dataset']['task'], **config["model"]).to(device)
            # model = _load_pre_trained_weights(model)
        elif config['model_type'] == 'gcn':
            from models.gcn_finetune import GCN
            model = GCN(config['dataset']['task'], **config["model"]).to(device)
            # model = _load_pre_trained_weights(model)
        ...
        _test(model_list, test_loader)

    def _load_pre_trained_weights(, model):
        try:
            checkpoints_folder = os.path.join('./ckpt', config['fine_tune_from'], 'checkpoints')
    ...
    def _test( model_list, test_loader):
