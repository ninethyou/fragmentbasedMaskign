...
class MolTestDatasetRich(InMemoryDataset):
    def __init__(self, data_path, target, task, mask_rate, mask_hop, transform=None, pre_transform=None, pre_filter=None):
    ...
    def __getitem__(self, index):
            ...
            removed_node_index, removed_edge_index = removeSubgraph(molGraph,  percent = self.mask_rate, max_hop = self.mask_hop)
...
class MolTestDatasetWrapperRich(object):
    ...
    def __init__(self, 
        ...
        data_path, target, task, splitting,seed, random_masking = 0, mask_rate = 0.2, mask_edge = 0, mask_hop = 1
    ):
    ...
    def get_data_loaders(self):
        ...
        train_dataset = MolTestDatasetRich(data_path=self.data_path, target=self.target, task=self.task, mask_rate = self.mask_rate, mask_hop = self.mask_hop)
