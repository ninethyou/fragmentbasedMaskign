...
def main():
    ...
    if args.dataset == 'zinc_standard_agent':
        ...
        from util import MaskAdjacent
        from batch import BatchMasking        
        
        dataset = MoleculeDataset(data_path = "dataset/" + args.dataset, )
        
        transform = MaskAdjacent(num_atom_type = 119, num_edge_type = 5, mask_rate = args.mask_rate, mask_edge=args.mask_edge) 
    ...
    output_file_temp = "./pretrain2/" + "/adjacent/" + f"{args.dataset}_{args.gnn_type}_{args.mask_rate}_{args.seed}"        
    ...
    for epoch in range(1, args.epochs+1):
        ...
        train_loss, train_acc_atom, train_acc_bond, epoch_element_counts_mol, epoch_element_counts_mask  = train(args, model_list, device, train_loader, optimizer_list, alpha_l=args.alpha_l, loss_fn=args.loss_fn, epoch= epoch)
    ...
    fn_base = f'{directory_name}/pretrain_adjacent_{args.dataset}_{args.split}'
