...
layout = {
    "recon": {
        ...
        "acc": ["Multiline", ["acc/acc_node", "acc/acc_edge"]],
    },
}
...
def plot_class_frequencies(predicted_classes, label, epoch ,accuracy):
    ...
    # Move tensors to CPU if they are on CUDA and convert to numpy arrays
    if predicted_classes.is_cuda:
    ...
    ax.set_title(f'Actual Label Frequencies at Epoch {epoch}\nAccuracy: {accuracy:.2f}')
    ax.set_xticks(index + bar_width / 2)
    ax.set_xticklabels(classes)
    ax.legend()

    # Ensure the directory exists
    directory = f'./plot/{args.task_name}/'
    if not os.path.exists(directory):
        os.makedirs(directory)

    # Save the plot to a file
    plt.tight_layout()
    plt.savefig(f'{directory}class_frequencies_{epoch}.png')  # Saves the plot as a PNG file
    plt.close(fig)  # Close the figure explicitly after saving it
...
class FineTune(object):
    ...
    def _step(self, model_list, data, n_iter):
        model, linear_pred_atoms, linear_pred_bonds, alpha = model_list
        ...
        masked_atom_indices = random_node(data.batch, args.mask_rate)



        print(masked_atom_indices.shape, data.batch.shape)
        ...
        if args.mask_edge:
            ...
            for bond_idx, (u, v) in enumerate(data.edge_index.cpu().numpy().T):
                for atom_idx in masked_atom_indices.tolist():
            ...
            mask_edge_label = mask_edge_label.to(pred_edge.device)
            ...
            # edge prediction   
            _, predicted_classes = torch.max(pred_edge, dim=1) # dim=1은 노드별로 최댓값을 찾음

            correct_predictions = (predicted_classes == mask_edge_label[:, 0])

            accuracy_edge = correct_predictions.float().mean()

            # node prediction
            _, predicted_classes = torch.max(pred_node, dim=1) # dim=1은 노드별로 최댓값을 찾음

            # 실제 라벨과 예측된 클래스가 일치하는지 확인합니다.
            correct_predictions = (predicted_classes == mask_node_label[:, 0])

            # 정확도를 계산합니다.
            accuracy_node = correct_predictions.float().mean()

        
        else: 
            masked_data = Data(x = masked_x ,edge_index = data.edge_index, edge_attr = data.edge_attr) 
        ...
        if args.mask_edge:
            ...
            total_loss = loss + alpha * (loss_recon_node + loss_recon_edge)

            return pred, total_loss, loss, loss_recon_node, loss_recon_edge, (accuracy_node, accuracy_edge), predicted_classes , mask_node_label[:, 0]
        else:
            total_loss = loss + alpha * loss_recon_node
            return pred, total_loss, loss, loss_recon_node, 0, (accuracy_node, 0), predicted_classes , mask_node_label[:, 0]
    ...
    def train(self):
        ...
        alpha  = torch.nn.Parameter(torch.tensor(args.alpha, dtype = torch.float), requires_grad=True)
        optimizer_alpha = torch.optim.Adam([alpha], lr=args.init_base_lr, weight_decay=args.weight_decay)
        ...
        model_list = [model, linear_pred_atoms, linear_pred_bonds, alpha]
        optimizer_list = [optimizer_model, optimizer_linear_pred_atoms, optimizer_linear_pred_bonds, optimizer_alpha]
        ...
        for epoch_counter in range(self.config['epochs']):
            predicted_classes_list = torch.tensor([], dtype=torch.float32).to(self.device) 
            mask_node_label_list = torch.tensor([], dtype=torch.float32).to(self.device)  # or torch.int64 if it's categorical labels
            ...
            for bn, data in enumerate(train_loader):
                ...
                pred, total_loss, loss_end, loss_recon, loss_recon_edge, acc, predicted_classes,  mask_node_label = self._step(model_list, data, n_iter)
                predicted_classes_list = torch.cat((predicted_classes_list, predicted_classes), dim=0)
                mask_node_label_list = torch.cat((mask_node_label_list, mask_node_label), dim=0)
                ...
                if n_iter % self.config['log_every_n_steps'] == 0:
                    ...
                    self.writer.add_scalar('alpha', alpha, epoch_counter)

                    if args.mask_edge == 1:

                        print(epoch_counter, bn, total_loss.item(),  acc[0].item(), acc[1].item())
                    else:

                        print(epoch_counter, bn, total_loss.item(),  acc[0].item(),)
            ...
            plot_class_frequencies(predicted_classes_list, mask_node_label_list,  epoch_counter, acc[0])
