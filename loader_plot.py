...
class MoleculeDataset(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        for key in self._data.keys():
            ...
            if key == 'mols':
                # Handle 'mols' separately, as it's not a tensor
    ...
    def collate(
        ...
        # Handling RDKit mol objects
        mols = [d.mols for d in data_list]
        data.mols = mols
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for i in range(len(smiles_list)):
                ...
                data = mol_to_graph_data_obj_simple(rdkit_mol)
                # # manually add mol id
                # data.id = torch.tensor( s = list(repeat(slice(None), item.dim()))
                #     [i])  # id here is the index of the mol in
                # the dataset
