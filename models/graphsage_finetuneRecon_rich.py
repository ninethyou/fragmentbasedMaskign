...
class GCNConv(MessagePassing):
    def __init__(self, emb_dim, aggr="add"):
        ...
        self.edge_embedding3 = nn.Embedding(8, emb_dim)
...
class GraphSAGEConv(MessagePassing):
    def __init__(self, emb_dim, aggr = "mean"):
        ...
        self.edge_embedding3 = torch.nn.Linear(8, emb_dim)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        edge_embeddings = self.edge_embedding1(edge_attr[:,0]) + self.edge_embedding2(edge_attr[:,1]) + self.edge_embedding3(edge_attr[:,2:].float())
...
class GraphSageRecon_richEmbedding(nn.Module):
    def __init__(self, task='classification', num_layer=5, emb_dim=300, feat_dim=256, drop_ratio=0, pool='mean',
                 pred_n_layer = 2, pred_act = 'softplus', num_task = 1):
        
        super(GraphSageRecon_richEmbedding, self).__init__()
        self.num_layer = num_layer
        self.emb_dim = emb_dim
        self.feat_dim = feat_dim
        self.drop_ratio = drop_ratio
        self.task = task
        self.num_taks = num_task
    ...
    def forward(self, data, mask_node = None):
        ...
        if mask_node is not None:
            ...
            feature_2_mask = self.x_embedding3(x[:,2:].float())
            ...
            feature_2_mask[mask_node] = torch.zeros(self.emb_dim).to(feature_2_mask.device)


            h = feature_0_mask + feature_1_mask + feature_2_mask
...
class GraphSage_rich(nn.Module):
    def __init__(self, task='classification', num_layer=5, emb_dim=300, feat_dim=256, drop_ratio=0, pool='mean',
                 pred_n_layer = 2, pred_act = 'softplus', num_task = 1):
        super(GraphSage_rich, self).__init__()
