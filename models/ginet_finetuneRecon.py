...
class GINEConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr, mask_edge = None):
        ...
        if mask_edge is not None:
            ...
            edge_0_mask = self.edge_embedding1(edge_attr[:,0])
            edge_1_mask = self.edge_embedding2(edge_attr[:,1])
            
            edge_0_mask[mask_edge] = torch.zeros(self.emb_dim).to(edge_0_mask.device)
            edge_1_mask[mask_edge] = torch.zeros(self.emb_dim).to(edge_1_mask.device)


            edge_embeddings = edge_0_mask + edge_1_mask
...
class GINetRecon(nn.Module):
    ...
    def __init__(self, 
        ...
        super(GINetRecon, self).__init__()
        self.num_layer = num_layer
        self.emb_dim = emb_dim
        self.feat_dim = feat_dim
        self.drop_ratio = drop_ratio
