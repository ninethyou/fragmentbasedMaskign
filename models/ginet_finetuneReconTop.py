...
class GINetReconEmbedding(nn.Module):
    ...
    def forward(self, data, mask_node = None):
        ...
        if mask_node is None:
            ...
            node = topk(score.squeeze(), 0.2, batch = data.batch)
...
class GINetReconTop(nn.Module):
    ...
    def __init__(self, 
        task='classification', num_layer=5, emb_dim=300, feat_dim=512, 
        drop_ratio=0, pool='mean', pred_n_layer=2, pred_act='softplus', num_task = 1, mask_rate = 0.2
    ):
        super(GINetReconTop, self).__init__()
        self.num_layer = num_layer
        self.emb_dim = emb_dim
        self.feat_dim = feat_dim
        self.drop_ratio = drop_ratio
    ...
    def forward(self, data, mask_node = None):
        ...
        if mask_node is None:
            score = self.dense_score(h_node).squeeze()
            node = topk(score.squeeze(), self.mask_rate, batch = data.batch)
