...
from dataset.dataset_test_adj import MolTestDatasetWrapper
...
class FineTune(object):
    ...
    def _step(self, model_list, data, n_iter):
        ...
        # get the prediction
        node_rep, pred, masked_atom_indices = model(data, mask_rate = args.mask_rate)  # [N,C]
        ...
        if args.mask_edge:
            ...
            node_repre2, output2_masked= model(masked_data, )
    ...
    def train(self):
        ...
        if self.config['model_type'] == 'gin':
            from models.ginet_finetuneReconBottom import GINetReconEmbeddingBottom
            model = GINetReconEmbeddingBottom(self.config['dataset']['task'], **self.config["model"]).to(self.device)
