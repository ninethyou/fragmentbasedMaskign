...
class FineTune(object):
    ...
    def _step(self, model_list, data, n_iter):
        ...
        masked_atom_indices_neighbor = get_neighbors(masked_atom_indices, data.edge_index)
        

        mask_node_labels_list = []
        for atom_idx in masked_atom_indices_neighbor:
            mask_node_labels_list.append(data.x[atom_idx].view(1, -1))

        mask_node_label = torch.cat(mask_node_labels_list, dim=0)
        masked_atom_indices = torch.tensor(masked_atom_indices)
        masked_atom_indices_neighbor = torch.tensor(masked_atom_indices_neighbor)
        ...
        if args.mask_edge:
            ...
            pred_node = linear_pred_atoms(node_rep_masked[masked_atom_indices_neighbor])
