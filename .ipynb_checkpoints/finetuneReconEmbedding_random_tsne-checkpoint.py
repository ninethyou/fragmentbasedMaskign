...
class FineTune(object):
    ...
    def _step(self, model_list, data, n_iter):
        ...
        if args.mask_edge:
            ...
            masked_data = Data(x = data.x ,edge_index = data.edge_index, edge_attr = masked_edge_attr  ) 

            node_rep_masked, output2_masked = model(data, mask_edge = connected_edge_indices)
    ...
    def train(self):
        ...
        optimizer_linear_pred_atoms = torch.optim.Adam(linear_pred_atoms.parameters(), lr=args.init_base_lr, weight_decay=eval(self.config['weight_decay']))
        optimizer_linear_pred_bonds = torch.optim.Adam(linear_pred_bonds.parameters(), lr=args.init_base_lr, weight_decay=eval(self.config['weight_decay']))
    ...
    def _test(self, model_list, test_loader):
        ...
        dir_tsne = os.path.join('plot_tsne',f'{args.task_name}', savefilename)
        ...
        merged_batch = medged_batch.to(self.device)
        with torch.no_grad():
            model.eval()
            pred_node, _ = model(medged_batch)
            graph_rep = self.pool(pred_node, medged_batch.batch)
            ...
            if config['dataset']['task'] == 'regression':
                ...
                for task_index in range(num_classes):
                    task_values = target_values[:, task_index]  # 현재 태스크에 대한 타겟값 가져오기
                    unique_classes = np.unique(task_values)  # 현재 태스크에 대한 유니크한 클래스 가져오기


                    file_name_tsne = f"{dir_name}_{config['dataset']['target'][task_index]}.png"
                    ...
                    plt.title(f'Latent Space for Task {task_index + 1}', fontsize=15)
                    plt.xlabel('Z1', fontsize=15)
                    plt.ylabel('Z2', fontsize=15)
                    plt.legend()
                    plt.savefig(full_file_name_tsne)  # 각 태스크에 대한 plot을 별도로 저장
                    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_decay', type=str, default='1e-6', help = "Weight decay")
