...
from dataset.dataset_test_rich_edge import MolTestDatasetWrapperRich
...
class FineTune(object):
    ...
    def _step(self, model_list, data, n_iter):
        ...
        if args.mask_edge:
            ...
            print(data.connected_edge_indices.shape)
            for edge_idx in data.connected_edge_indices:
                masked_edge_attr[edge_idx] = torch.zeros(10).to(self.device)
                masked_edge_attr[edge_idx, 0] = num_edge_type


            masked_data = Data(x = masked_x ,edge_index = data.edge_index, edge_attr = masked_edge_attr , x_add = data.x_add ) 

            node_rep_masked2, output2_masked = model(data)

            # edge_rep 
            edge_rep = node_rep_masked2[masked_edge_index[0]] + node_rep_masked2[masked_edge_index[1]]


            pred_edge = linear_pred_bonds(edge_rep)


            data.mask_edge_label = data.mask_edge_label.to(pred_edge.device)
            loss_recon_edge = self.criterion_recon(pred_edge.float(),  data.mask_edge_label[:,0])
            ...
            #embed masked node
            # masked_data = Data(x = data.x ,edge_index = data.edge_index, edge_attr = data.edge_attr, x_add = data.x_add, batch = data.batch) 
            masked_data = data.clone().to(self.device) 

            node_rep_masked2, output2_masked = model(masked_data, data.masked_atom_indices)   
            # node_rep_masked, output2_masked = model(masked_data)   

        pred_node = linear_pred_atoms(node_rep_masked2[data.masked_atom_indices])

        loss_recon_node = self.criterion_recon(pred_node.float(), data.mask_node_label[:,0].long())
    ...
    def train(self):
        ...
        if self.config['model_type'] == 'gcn':
            from models.gcn_finetuneRecon_rich import GCNRecon_rich
            model = GCNRecon_rich(self.config['dataset']['task'], **self.config["model"]).to(self.device)
