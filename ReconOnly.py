...
layout = {
    "recon": {
        ...
        "accuracy": ["Multiline", [ "accuracy/validation_recon_node", "accuracy/validation_recon_edge"]],
    },
}
...
def calc_acc_edge(pred, label):
            # edge prediction   
        _, predicted_classes = torch.max(pred, dim=1) # dim=1은 노드별로 최댓값을 찾음

        correct_predictions = (predicted_classes == label[:, 0])
...
class FineTune(object):
    ...
    def _step(self, model_list, data, n_iter):
        ...
        if args.mask_edge:
            ...
            return total_loss, loss_recon_node, loss_recon_edge, calc_acc_edge(pred_node, data.mask_node_label), calc_acc_edge(pred_edge, mask_edge_label)
        else:
            ...
            total_loss = loss_recon_node
            return total_loss, loss_recon_node, 0, calc_acc_edge(pred_node, data.mask_node_label),0
    ...
    def train(self):
        ...
        for epoch_counter in range(self.config['epochs']):
            ...
            #     self.writer.add_scalar('acc/validation_recon_node', valid_cls_node, epoch_counter)
            #     self.writer.add_scalar('acc/validation_recon_edge', valid_cls_edge, epoch_counter)
    ...
    def _validate(self, model_list, valid_loader):
        ...
        with torch.no_grad():
            ...
            for bn, data in enumerate(valid_loader):
                ...
                if args.mask_edge:
                    loss, pred_node,mask_node_label0, pred_edge,mask_edge_label  = self._step_test([model, linear_pred_atoms, linear_pred_bonds], data, bn)
        ...
        if args.mask_edge:
            ...
            labels_edge = torch.cat(labels_edge, dim=0).cpu().numpy()
            predictions_edge = torch.cat(predictions_edge, dim=0).cpu().detach().numpy()

            labels_node = torch.cat(labels_node, dim=0).cpu().numpy()
            predictions_node = torch.cat(predictions_node, dim=0).cpu().detach().numpy()

            predictions_edge = np.array(predictions_edge)
            labels_edge = np.array(labels_edge)

            predictions_node = np.array(predictions_node)
            labels_node = np.array(labels_node)


            _, predicted_classes = torch.max(predictions_node, dim=1) # dim=1은 노드별로 최댓값을 찾음

            # 실제 라벨과 예측된 클래스가 일치하는지 확인합니다.
            correct_predictions = (labels_node == predictions_node)

            # 정확도를 계산합니다.
            accuracy_node = correct_predictions.float().mean()

            _, predicted_classes = torch.max(predictions_edge, dim=1) # dim=1은 노드별로 최댓값을 찾음

            # 실제 라벨과 예측된 클래스가 일치하는지 확인합니다.
            correct_predictions = (predicted_classes == predictions_edge)
            ...
            _, predicted_classes = torch.max(labels_node, dim=1) # dim=1은 노드별로 최댓값을 찾음

            # 실제 라벨과 예측된 클래스가 일치하는지 확인합니다.
            correct_predictions = (predicted_classes == predictions_node)

            # 정확도를 계산합니다.
            accuracy_node = correct_predictions.float().mean()

            _, predicted_classes = torch.max(labels_edge, dim=1) # dim=1은 노드별로 최댓값을 찾음
    ...
    def _test(self, model_list, test_loader):
        ...
        with torch.no_grad():
            ...
            for bn, data in enumerate(test_loader):
                ...
                pred_node = linear_pred_atoms(node_rep[data.masked_atom_indices])
                loss_recon_node = self.criterion_recon(pred_node.float(), data.mask_node_label[:,0])

                acc_node =  calc_acc_edge(pred_node, data.mask_node_label),


                valid_loss += loss_recon_node.item() * data.y.size(0)
                num_data += data.y.size(0)

            valid_loss /= num_data

            print('Test loss:', valid_loss, 'Test ROC AUC:', acc_node)
